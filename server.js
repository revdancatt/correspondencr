...
if (!fs.existsSync(path.join(rootDir, '.env'))) {
  ...
  let port = prompt('port number (8080): ')

  if (port === '') port = 8080
  port = parseInt(port)
  if (isNaN(port) || port < 0 || port > 49151) {
    console.log('Port must be between 0-49151, setting port to 8080'.alert)
    port = 8080
  }
 ...
 *
 */
if (!fs.existsSync(path.join(rootDir, 'src'))) fs.mkdirSync(path.join(rootDir, 'src'))
if (!fs.existsSync(path.join(rootDir, 'src', 'templates'))) fs.mkdirSync(path.join(rootDir, 'src', 'templates'))
if (!fs.existsSync(path.join(rootDir, 'src', 'public'))) fs.mkdirSync(path.join(rootDir, 'src', 'public'))
if (!fs.existsSync(path.join(rootDir, 'src', 'public', 'images'))) fs.mkdirSync(path.join(rootDir, 'src', 'public', 'images'))
if (!fs.existsSync(path.join(rootDir, 'src', 'public', 'css'))) fs.mkdirSync(path.join(rootDir, 'src', 'public', 'css'))
